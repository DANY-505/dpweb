        ...
        if (json.status && json.data && json.data.length > 0) {
            ...
            json.data.forEach((user, index) => {
                html += `<tr>
                    ...
                    <td>${user.estado || ''}</td>
                    <td>
                        <button onclick="window.location.href='`+ base_url + `proveedor-edit/` + user.id + `'" class="btn btn-edit">Editar</button>
            ...
            document.getElementById('content_proveedor').innerHTML = '<tr><td colspan="6">No hay proveedores disponibles</td></tr>';
        }
    } catch (error) {
        console.log(error);
        document.getElementById('content_proveedor').innerHTML = '<tr><td colspan="6">Error al cargar los proveedores</td></tr>';
    }
}
